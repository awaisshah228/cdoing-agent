...
// ── Line ending utilities (from opencode) ──────────────────────────────────
...
- This tool CANNOT be called in parallel with itself on the SAME file.`,
  ...
  async execute(input: Record<string, unknown>): Promise<ToolResult> {
    ...
    // Detect and preserve original line endings (Windows \r\n vs Unix \n)
    ...
    try {
      ...
      if (isUnifiedDiff(oldStr)) {
        const result = applyUnifiedDiff(content, normalizeLineEndings(oldStr));
        fs.writeFileSync(filePath, convertToLineEnding(result, originalEnding), "utf-8");
        return { success: true, output: `Applied unified diff patch to ${filePath}\n\n${oldStr}` };
      }

      // Normalize line endings in search/replace strings to match file
      const normalizedOld = normalizeLineEndings(oldStr);
      const normalizedNew = normalizeLineEndings(newStr);

      // Lazy apply: expand placeholder markers in new_string before replacing
      let expandedNewStr = normalizedNew;
      let lazyNote = "";
      if (hasPlaceholders(normalizedNew)) {
        const { content: expanded, placeholdersExpanded, strategy: lazyStrategy } = expandPlaceholders(content, normalizedNew);
      ...
      const { result, count, strategy } = executeFindAndReplace(content, normalizedOld, expandedNewStr, replaceAll);
      // Convert back to original line endings before writing
      fs.writeFileSync(filePath, convertToLineEnding(result, originalEnding), "utf-8");
